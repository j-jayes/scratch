---
title: "Swedish daily newspapers data parser"
format: 
    pdf:
...
I want to get information about the jobs advertised in Sweden to track how descriptions for jobs changed over time. I was inspired by the paper [*Running out of time: using job ads to analyse the demand for messengers in the twentieth century* by Håkansson, Karlsson and La Mela](https://doi.org/10.1080/03585522.2022.2106300) that focusses specifically on messengers, and uses data from the *Svenska dagstidningar* database described by Karlsson [here](https://www.historisktidskrift.se/index.php/june20/article/view/291/245).

Luckily, a number of the newspapers are digitized by the [Royal Library of Sweden](https://tidningar.kb.se/).
...
```{r}
knitr::include_graphics(here::here("images/Svenska-dagstidningar.png"))
```
...
```{r}
library(XML)
xml <- XML::xmlParse(here::here("data/bib4345612_18730208_0_32_0001_alto.xml"))
...
```{r}
knitr::include_graphics(here::here("images/newspaper_1.png"))
```


```{r}
xml <- XML::xmlParse(here::here("data/bib13991099_18700805_0_1709_0001_alto.xml"))
...
test %>%
  ...
  filter(id_k == "STR") %>% 
  select(VPOS, WIDTH, HEIGHT, CONTENT, WC)
```
...
```{r}
file_path <- here::here("data/bib13434192_18990409_0_95_0005_alto.xml")
...
parse_xml <- function(file_path) {
  ...
  get_paper_text <- function(tb) {
    ...
    tbl_2 %>%
      ...
      mutate(across(c("HPOS", "VPOS", "WIDTH", "HEIGHT"), as.numeric))
  }
...
test %>%
  ...
  knitr::kable(col.names = c("Text block", "Text"))
...
viz_text_raw <- function(tbl, tb) {
  test %>%
    ...
    geom_text() +
    labs(title = "Location of text on page")
}
